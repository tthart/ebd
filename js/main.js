...
$(document).on('click', '.toggle', function(){
   $(this).nextUntil('.toggle').toggleClass("hidden");	
   $(this).children('i').toggleClass("fa-plus fa-minus");
   console.log(document.getElementsByTagName('.toggle').length);
});




$("#searchButtonPeople").click(function() {
	//Call query for people
	//$('#searchResults').html("<p>Test</p>");
	search("person");
	
	
});

$("#searchButtonCorp").click(function() {
	//Call query for people
	//$('#searchResults').html("<p>Test</p>");
	search("corp");
});

$("#searchButtonPlace").click(function() {
	//Call query for people
	//$('#searchResults').html("<p>Test</p>");
	search("place");
...
var href;
	var query = $("#searchBox").val();
		...
		var api = traverson
		.from("http://54.72.154.243/ebd/"+page+"s?q=" + query	) // set root URL
		...
		//	.follow('corps')
			.getResource(function(error, resource, traversal) {
				...
				var prev;
					...
					var i = 0;
					var html = "";
					html+='<div class="large-12 columns">You searched for: ' + query + '</div>';
					var obj = resource["_embedded"][page];
					for (var key in obj) {
					if(i % 5 == 0) { 
					html += "</ul></div><div class='large-3 columns'><ul>";
					}
					console.log(resource._embedded.page);
						
						var decodeU = encodeURIComponent(obj[key]["_links"]["self"]["href"]);
					/*Embedded*/
					/*	if (resource[key]._embedded) {
						*/
						
							
							
						if(page=="corp"){
								html += "<li><a href='/organisation/index.html?id=" + decodeU + "'>" + obj[key]["displayName"] + "</a>";
								if(obj[key]._links.hubRec){html+=" <span class='label success'>Featured</span>"}
								console.log(decodeU);
								html+="</li>";
							}
							else if(page=="place")
							{
								html += "<li><a href='/place/index.html?id=" + decodeU + "'>" + obj[key]["label"] + "</a>";
								console.log(decodeU);
								if(obj[key]._links.hubRec){html+=" <span class='label success'>Featured</span>"}
								html+="</li>";
							}
							...
							{
							html += "<li><a href='/"+page+"/index.html?id=" + decodeU + "'>" + obj[key]["displayName"] + "</a>";	
							if(obj[key]._links.hubRec){html+=" <span class='label success'>Featured</span>"}
							html+="</li>";
							}
...
{
f = dateFormat(dateString,"yyyy")
} 
else if (dateString.length==10) {
f = dateFormat(dateString,"dd mmm yyyy")	
}else 
{
f = dateFormat(dateString,"mmm yyyy");	
}//console.log("original: " + obj[key].date + " Split: " + obj[key].date.split("-"));
return f;
}